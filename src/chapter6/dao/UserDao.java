...
public class UserDao {
	...
	*/
	...
	public void insert(Connection connection, User user) {
		...
		PreparedStatement ps = null; //SQLの下地を作る
		try {
			...
			ps = connection.prepareStatement(sql.toString());//SQL文を実行できるようにする

			ps.setString(1, user.getAccount());//SQLの動的部分に値をセットする
			ps.setString(2, user.getName());
			ps.setString(3, user.getEmail());
			ps.setString(4, user.getPassword());
			ps.setString(5, user.getDescription());

			ps.executeUpdate();//更新系SQLの場合は、executeUpdate()を呼び出すことでSQLが実行される
	...
	public User select(Connection connection, String accountOrEmail, String password) {
		...
		try {
			...
			ps = connection.prepareStatement(sql); // ?(プレースホルダー)と組み合わせて使用する

			ps.setString(1, accountOrEmail); // ?(プレースホルダー)に設定する値
